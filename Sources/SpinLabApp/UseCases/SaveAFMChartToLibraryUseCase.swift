...
// MARK: - SaveAFMChartInput

struct SaveAFMChartInput: Sendable {
    var png: Data
    var projection: AFMSaveProjection
...
// MARK: - SaveAFMChartToLibraryUseCase

/// AFM-specific save-to-Library use case. Mirrors `SaveRSMChartToLibraryUseCase` exactly
/// (same underlying Library artifact/index primitives), taking `AFMSaveProjection` instead of
/// `RSMSaveProjection`. The common Save Module (`SaveActiveChartToLibraryUseCase` +
/// `PersistChartArtifactUseCase`) is not modified and does not infer AFM semantics.
struct SaveAFMChartToLibraryUseCase {
    ...
    func execute(input: SaveAFMChartInput) -> PersistenceOutcome {
        ...
        var semanticParams = proj.semanticParams
        semanticParams["channelID"] = proj.activeChannelID
        if let channelSemanticType = proj.channelSemanticType {
            semanticParams["channelSemanticType"] = channelSemanticType
        }
        ...
        let safeTitle = rawTitle.isEmpty ? "afm" : rawTitle
        ...
        do {
            ...
            return .failure(AppError.from(error, fallback: "AFM chart persist failed").localizedDescription)
        }
    }
}
