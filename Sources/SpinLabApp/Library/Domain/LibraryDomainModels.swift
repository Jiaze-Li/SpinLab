...
// MARK: - Tier 2 Library domain entities
// Codable/Hashable/Sendable pure value contracts shared across Library persistence,
// data actor, and UseCase layers. Not for View-only consumption.
...
struct LibraryWarning: Identifiable, Codable, Hashable, Sendable {
    var id: UUID = UUID()
    var message: String
    var affectedSampleKey: String?
    var severity: WarningSeverity = .warning

    enum WarningSeverity: String, Codable, Hashable, Sendable {
        case warning
        case error
    }
}

enum LibraryManualLogStatus: String, Codable, CaseIterable, Hashable, Sendable {
    case pending
    case done
    case ignored
}

struct LibrarySampleChangeLogItem: Codable, Hashable, Sendable {
    var key: String
    var oldValue: String?
    var newValue: String?
}

struct LibrarySampleChangeLogEntry: Identifiable, Codable, Hashable, Sendable {
    var id: UUID
    var sampleId: String
    var batchId: String
    var changedAt: Date
