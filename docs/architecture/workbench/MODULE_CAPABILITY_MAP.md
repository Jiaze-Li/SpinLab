...
| Dual-axis render path | Plot System dual-axis path | Common module group | `modules/PLOT_SYSTEM.md`, `WORKFLOW_EXTENSION.md` | none yet — new capability | `Workbench/Modules/PlotSystem/DualAxis/` | DualAxis is parallel to Cartesian XY and Heatmap. Must not extend `WorkbenchPlotPayload` for secondary-axis data. Workflow-specific semantics (RAHE, σxx, Temperature Dependence, 3ω) enter only through explicit DualAxis payloads. |
