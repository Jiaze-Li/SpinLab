...
| Series overlay contract | Plot System | Common module group | `modules/PLOT_SYSTEM.md` | `WorkbenchPlotSeriesOverlay.swift`, `WorkbenchRenderPipeline.swift`, `WorkbenchChartRenderer.swift` | `Workbench/Modules/PlotSystem/Contracts/` + `Workbench/V3/` | Parent-anchored overlays are display-only additions. They must stay out of legend inference, reorder, and stacking semantics. |
...
| Generic power-law fit use case | PowerLawFit | Optional module | `modules/POWER_LAW_FIT.md`, `MODULE_BOUNDARIES.md` | `Workbench/Modules/PowerLawFit/*` | `Workbench/Modules/PowerLawFit/` | Reusable numeric fit engine; no IV, workflow, unit, channel, or rendering semantics. |
