...
| Cartesian XY plot payload contract | Plot System | Common module group | `modules/PLOT_SYSTEM.md`, `WORKFLOW_EXTENSION.md` | `Workbench/V3/WorkbenchResultContracts.swift` | `Workbench/Modules/PlotSystem/Contracts/` | Payload carries x/y series arrays, axis mapping, labels, metadata, and capability flags. |
| Render pipeline | Plot System | Common module group | `modules/PLOT_SYSTEM.md` | `Workbench/V3/WorkbenchRenderPipeline.swift` | `Workbench/Modules/PlotSystem/Pipeline/` | Pipeline orchestrates style merge, legend auto-resolution, series reversal/order checks, and PNG output. |
| Legend auto-resolution / automatic legend labels | Plot System distributed legend capability | Common module group | `modules/PLOT_SYSTEM.md`, `WORKFLOW_EXTENSION.md` | `Workbench/Modules/PlotSystem/Legend/LegendDimensionResolver.swift`, `WorkbenchRenderPipeline.swift`, `WorkbenchResultContracts.swift` | `Workbench/Modules/PlotSystem/Legend/` | Resolver reads `WorkbenchPlotSeries.metadata`. It must not parse filenames, sidecars, search hits, or workflow-local state. |
...
| Copy PNG / scale menu | Plot Canvas + render pipeline | Common module group | `modules/PLOT_SYSTEM.md` | `WorkbenchPlotCanvas.swift`, `WorkbenchRenderPipeline.swift`, workflow stores' copy callbacks | `Workbench/Modules/PlotSystem/Canvas/` + `Pipeline/` | 2x may reuse cached image; 1x/3x re-render through pipeline. |
| Plot controls panel | Plot Controls | Common module group | `modules/PLOT_SYSTEM.md` | `WorkbenchPlotControlsPanel.swift`, `WorkbenchStandardPlotControls.swift` | `Workbench/Modules/PlotSystem/Controls/` | Shared control host; workflow-specific controls must enter through explicit slots/bindings. |
...
| copy image, export PNG, scale PNG | Copy PNG / scale menu |
