...
| Copy PNG / scale menu | Plot Canvas + render pipeline | Common module group | `modules/PLOT_SYSTEM.md` | `WorkbenchPlotCanvas.swift`, `Workbench/Modules/PlotSystem/Pipeline/WorkbenchRenderPipeline.swift`, workflow stores' copy callbacks | `Workbench/Modules/PlotSystem/Canvas/` + `Pipeline/` | 2x may reuse cached image; 1x/3x re-render through pipeline. |
