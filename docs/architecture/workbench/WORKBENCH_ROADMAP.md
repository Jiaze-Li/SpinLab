...
- **Audit findings summary**:
  - Safe: session-only fields (persistenceOutcome, saveMessage, currentRunTrace) confirmed not serialized and nil after restore. Overlay state confirmed not serialized and cleared on restore. Save coordinator confirmed not creating pack/restore state. Cross-workflow isolation confirmed. RT slot isolation confirmed.
  - Dangerous: `cachedRTFilePath` write-then-overwrite in `restoreFromPack` — line 155 writes `config.rtFilePath` → `cachedRTFilePath`, then `_snapshotAndCacheManifestPayloads()` immediately overwrites it. Effective RT restore is via `selectedRTHit` only; `config.rtFilePath` is a fingerprint-only field in practice.
  - Gap 1 (HIGH): no test for the `cachedRTFilePath` overwrite sequence in the non-nil selectedRTHit case.
  - Gap 2 (MEDIUM): no test for 3ω or XY required-field decode failure path in `loadPack`.
  - Gap 3 (LOW): AHE / XY missing optional fields backward-compat not explicitly tested.
- **Gate 7.6A test targets**: 6 new baseline tests before extraction — see audit doc §8.
- **Gate 7.6B extraction / fix targets**: decide on `cachedRTFilePath` standalone-vs-derived; `_overlayPackIDs` standalone fallback cleanup — see audit doc §8.
