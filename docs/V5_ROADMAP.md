...
| s10 [~] 进行中 | Sample Identification 面板二次简化：(1) Token Separators 字段删除（commit 880aab4，2026-04-27）；(2) 删除 Substrate Tag Rules + 三表字段精简 + UI 重写 + Sample ID 退正则用 prefix 列表（双盲对抗已收敛，handoff `2026-04-27-5.1.5-s10-substrate-redesign.md` 待消费） | TBD |

总计约 **44–66 h** 跨 7 对话（s1–s9，s8 是设计对话不计工作量）。s5 / s9 是工作量最大的执行会话。s10 在 s9 落地后追加，工作量待 Codex 评审后估。

##### s10 设计纪要（2026-04-27 需求确认完成）

**动机**：s9 落地后用户使用 Sample Identification 面板时反馈面板过度工程化、概念冗余："Substrate Tag Rules 和 Substrate Configuration 干同一件事"、"Token Separators 和 Filename Tokenization 那栏重复"、"id 和 token 是一个功能"、"为什么有 equals/equalsAny/contains/containsAny/equalsOrContainsAny 五种类型"、"^(PN|PT|SL)\d+$ 我看不懂"、"\d{3} 我也看不懂"。s10 一次性把这些冗余/不可读概念清掉。

**(1) Token Separators 字段删除（已落地，commit 880aab4）**

- Sample Identification 那栏的 Token Separators 输入框删除
- substrate token 切分逻辑统一改读 `filename_tokenization.json` 的 `tokenization.separators`
- sample_identification schema bump v2 → v3，V2→V3 迁移：删除 `substrate.tokenSeparators` 键；旧值与 filename_tokenization 不一致写 warning 入 migration_state
- migration_state.rules_schema_version gate 同步 2 → 3
- shipped sample_identification.json 升 v3 并去字段
- V515 测试 fixture/断言全部升 v3，新增 4 条覆盖：v2→v3 separator 一致/不一致、state v2 重入、原 v2-only 检查改 v3-only
- Codex 评审：adopt-with-fixes，5 条修复全部纳入

**(2) Substrate Tag Rules 删除 + 三表字段精简 + UI 重写（待 Codex 实施方案对抗）**
...
[4] Orientations
    ▼ 111
        ├─ (displayName 自带 equals 自匹配)
        └─ [+ Add match]
```

**锁定的设计要点**：

- 每行 head = displayName = canonical 标签（导入时贴在文件上的 substrate tag）
- displayName 自带 `equals` 自匹配，不需要在 matches 列表里再列一遍
- 展开后是 match conditions 列表，多条 OR 关系（任一命中算这一行命中）
- 匹配类型只两种：`equals` / `contains`（删除 `equalsAny` / `containsAny` / `equalsOrContainsAny` / `regex`；多类型在新 UI 用多条匹配自然 OR 取代）
- 删除 `scope` 选项，默认 token（每条 match 在切碎后的 token 列表上逐块匹配）
- Batch ID 改用 prefix 列表（chip-style），程序内部按 `prefix + 数字` 构建匹配规则；用户视角看不到正则
- Orientations 删除 `\d{3}` regex pattern，靠每行显式列出（001/110/111/0001）
- 复合 token (STO111/STO001) 处理：用户在多张表分别加 `contains` 条件——Materials/STO 加 `contains "STO111"` + Orientations/111 加 `contains "STO111"`；功能等价于旧 Tag Rules

**取消的概念**：

- ❌ Substrate Tag Rules 整组（UI + 数据模型 + shipped 规则 + 旧迁移路径）
- ❌ id / tokens / aliases 三个独立字段（合并为：displayName + matches[]）
- ❌ scope 选项（默认 token）
- ❌ equalsAny / containsAny / equalsOrContainsAny / regex 四种匹配类型
- ❌ orientations.pattern 字段（`\d{3}` 正则）
- ❌ sampleId.patterns 显式正则（`^(PN|PT|SL)\d+$`）→ 改为 batchPrefixes 字符串列表

**对外行为不变（验收口径）**：

- Library 详情页"Substrate Tags"显示一致
- 多衬底样品 routing 消歧能力一致（标签来源换了，呈现不变）
- 现有 13 条 shipped substrateTagRules 全部能在新结构里表达（11 条转 displayName 自匹配或 contains/equals；2 条复合 token 拆为多张表 contains）

**新增逻辑**：

- 替代 `ruleSet.substrateTags(from:)`：扫文件名 token，命中任一表的任一 entry 的任一 match → 贴 entry 的 displayName 为标签

**数据迁移（v3 → v4）**：

- 旧 entry `{id, tokens, aliases, displayName}` 合并：`displayName` 保留为新 displayName；`tokens` 全部转为 `equals` matches；`aliases` 全部转为 `equals` matches；`id` 丢弃；去重
- treatments 旧字段 `keywords` / `standaloneTokens` / `containsTokens` 同样合并：standaloneTokens → equals，containsTokens → contains，keywords → 按当前语义 contains
- 旧 `substrateTagRules` 全部删除（warning 入 migration_state，列出每条丢弃的 rule 供用户复核）
- 旧 `sampleId.patterns` 解析提取 prefix 列表写入新 `sampleId.batchPrefixes`；非 `^(prefix|prefix|...)\d+$` 形态的 pattern 写 warning + 丢弃
- 旧 `orientations.pattern` 字段丢弃
- shipped sample_identification.json 同步重写为 v4

**实施风险点（待 Codex 评审）**：

- `FileRoutingSemanticRules.load` 当前从 substrateTagRules 推导 treatmentNeedles/materialNeedles/orientationNeedles，删 Tag Rules 后此路径需重写
- `ruleSet.substrateTags(from:)` 是 11 个下游消费者的源头（Library 详情页 / RoutePlanner / InboxRoutingState 等），新生成器输出形态必须与旧 Tag Rules 输出形态等价
- chip-style UI 是 macOS SwiftUI 上较新组件，需评估是否要自实现还是用现成 Token Field
- 迁移 `sampleId.patterns` 的 prefix 提取需要正则解析正则（脆弱），可能要保留"无法解析时回落到原 patterns 列表"出口

**否决方案及理由**（不要后续 agent 推翻）：

- ❌ 只保留 Substrate Tag Rules 一张大映射表 —— 丢分类信息（routing 多衬底消歧依赖材料/方向区分），要在表内重建分类 = 倒退
- ❌ 只删 UI 不删机制 —— 留半成品双源；按 Jack 一贯态度（"现在有的清理掉，全部重新做，不要在烂地里改良"）不接受
- ❌ 推迟到 5.1.6+ —— 已经在这条线上有热的上下文，独立 session 重新装填成本高
- ❌ 保留 equalsAny / containsAny / regex —— 多条件 OR + 两类型已覆盖；regex 在 shipped 配置 0 处使用
- ❌ 保留 scope 选项 —— shipped 0 处使用 joined；用户看不懂
- ❌ 保留 sampleId 显式 regex —— 用户看不懂；prefix 列表表达力够用
