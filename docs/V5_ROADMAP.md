...
| s7 ✅ 已归档 | 任务 2/3/4 落地：WorkflowDefinitionStore（读 config/workflow.json）+ WorkflowRegistryRetirementService（外层 registry 合并）+ parentID 兼容码删除 + RulesBootstrapper 替换 RulesMigration + WorkflowRegistryView 只读重写；5 commit（08c7f8a–f7939c4）+ 11 tests（79/79 V515 green） | — |
...
**状态**：`[~]` 进行中。设计已收敛（双 AI 对抗 + Jack 拍板），handoff 待落盘。

**动机**：跨版本任务态散落在 V5_ROADMAP / handoff/README / TECH_DEBT_BACKLOG 三份文档，每份只看流水线一段切片，回到项目无法一眼判断"当下在做哪些版本任务、各自卡在哪一阶段"。同时 TECH_DEBT_BACKLOG 长期只装一两条 raw 捕获、handoff/README 内容职责分裂（待消费索引 + 已归档 audit + tmp protocol + 分工样板 + 命名指针 5 类内容混塞），都已退化为低维护价值文档。
...
**E. 全局 workflow.md 配套改动（规范类，需 Jack 事前审过）**
...
详见 `docs/handoff/2026-04-26-5.7.1-task-board-introduction.md`。要点：

- **Commit 1（项目仓库，原子）**：建 TASK_BOARD + INDEX + 删 handoff/README + 删 TECH_DEBT + 改 docs/README.md + 改项目 CLAUDE.md 4 处（吸收 Codex 终审 F1：旧落点删除与新指针切换原子化，消除中间态）
- **Commit 2（dotfiles 仓库）**：改全局 workflow.md 5 处 + dotfiles push（独立 repo，与项目 commit 时序解耦）
