...
struct V5115RulesBootstrapperCharacterizationTests {
    ...
    //
    // Each test runs against a tempDir, never the real ~/Library/Application Support/SpinLab/config.
    // The previous "acquireIsolation" pattern (mv real dir → write fixtures → mv back) was a v5.1.15
    // round-1 carryover that violated the single-user-app rule (CLAUDE.md "Product Scope" §4): a test
    // crash / timeout / concurrent run could leave the user's real rules dir renamed and absent.
    // Fix: inject `RulesConfigPaths(configDirectoryURL:)` and pass it explicitly to
    // `RulesBootstrapper.migrateRuntimeRulesIfNeeded(paths:)`. Mirror of the v5.1.15h
    // LibrarySettingsStore double-init treatment.

    @discardableResult
    private func withIsolatedRulesConfig<T>(
        _ body: (RulesConfigPaths) throws -> T
    ) throws -> T {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("V5115RB-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
        let paths = RulesConfigPaths(configDirectoryURL: tempDir)
    ...
    func stateV7ButMeasuringV6TriggersReMigration() throws {
        try withIsolatedRulesConfig { paths in
            try self.seedV6MeasuringCondition(at: paths.measuringConditionURL)
    ...
    func decodeVerifyFailurePreservesOriginalsAndWritesFailureFile() throws {
        try withIsolatedRulesConfig { paths in
            ...
            try self.seedV4MeasuringConditionWithInvalidMatchType(at: paths.measuringConditionURL)
            try self.seedV5SampleIdentification(at: paths.sampleIdentificationURL)
            ...
            let contents = try FileManager.default.contentsOfDirectory(atPath: paths.configDirectoryURL.path)
