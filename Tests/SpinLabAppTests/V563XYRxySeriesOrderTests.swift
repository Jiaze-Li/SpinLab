...
@Suite("V5.6.3 XY Rxy Series Order")
struct V563XYRxySeriesOrderTests {
    ...
    private func makeSweeps() -> [XYRotationAngleSweep] {
        [
            makeSweep(stem: "bottom", temperatureK: 5, resistanceXY: [0, 1, 2]),
            makeSweep(stem: "middle", temperatureK: 10, resistanceXY: [10, 11, 12]),
            makeSweep(stem: "top", temperatureK: 15, resistanceXY: [20, 21, 22])
        ]
    }
    ...
    private func reversedRequestedOrder() throws -> [String] {
        let manifest = try #require(XYRotationPlotRenderer().makeRxyVsPhiPayload(
            sweeps: makeSweeps(),
            device: "0deg"
        ))
    ...
    @Test("XY Rxy stacked payload consumes full identity-key reorder")
    ...
    @MainActor
    @Test("XY Rxy chips, legend, and display share one identity order")
    func chipLegendDisplayAlignment() throws {
        ...
        let (_, _, layout, displayPayload, warnings) = XYRotationRenderRoute.renderRxyVsPhiViaSharedRoute(
    ...
    @MainActor
    @Test("XY Rxy stacked display keeps descending mean-y order under reordered input")
    func meanYDescendingOrderMatchesVisualOrder() throws {
        let sweeps = makeSweeps()
        let requestedOrder = try reversedRequestedOrder()

        let (_, _, _, displayPayload, warnings) = XYRotationRenderRoute.renderRxyVsPhiViaSharedRoute(
            renderer: XYRotationPlotRenderer(),
            sweeps: sweeps,
            device: "0deg",
            seriesOrder: requestedOrder
    ...
    @MainActor
    @Test("XY Rxy hidden filtering compacts stack and preserves order")
    ...
    @MainActor
    @Test("XY Rxy all-hidden behavior keeps old warning and visible series")
    ...
    @Test("XY Rxy render path emits no seriesOrder mismatch warning")
    ...
    @MainActor
    @Test("XY Rxy pack restore preserves series order")
    func packRestorePreservesSeriesOrder() async throws {
        ...
        store.tabs.tabStates[.rxyVsPhi] = TabRenderState(seriesOrder: requestedOrder)

        let config = XYRotationPackConfig(
            phiOffsetOverrides: [:],
            centerBaseline: false,
            linearDetrend: false,
            activeTab: XYRotationWorkbenchTab.rxyVsPhi.rawValue,
            ...
            tabStates: [XYRotationWorkbenchTab.rxyVsPhi.rawValue: TabRenderState(seriesOrder: requestedOrder)],
        ...
        #expect(restored.tabs.state(for: .rxyVsPhi).seriesOrder == requestedOrder)

        for _ in 0..<40 {
            let output = restored.tabs.output(for: .rxyVsPhi)
        ...
        Issue.record("Timed out waiting for XY Rxy pack restore to preserve series order")
    }
}
